...
fill up the indices in the prefi    for(int i=0; i<n; i++){
        for(int j=0; j<=i; j++){
            prefix_sum[i]+=ara[j];
        }
    }x_sum array. For each index i from 0 to r-1, and for
...
void build_prefix_sum_array_2D(int r, int c, int ara[1005][1005], int prefix_sum[1005][1005]){
    prefix_sum[0][0] = ara[0][0];
    for (int i = 1; i < c; i++)
        prefix_sum[0][i] = prefix_sum[0][i - 1] + ara[0][i];
    for (int i = 0; i < r; i++)
        prefix_sum[i][0] = prefix_sum[i - 1][0] + ara[i][0];
    for(int i=1;i<r;i++){
        for(int j=1; j<c; j++){
        prefix_sum[i][j] = prefix_sum[i - 1][j] + prefix_sum[i][j - 1]  - prefix_sum[i - 1][j - 1] + ara[i][j];
        }
    }
}
...
void get_original_array(int n, int prefix_sum[], int ara[]){
    ara[0]=prefix_sum[0];
    for(int i=1; i<n; i++){
        ara[i]=prefix_sum[i]-prefix_sum[i-1];
    }
}
